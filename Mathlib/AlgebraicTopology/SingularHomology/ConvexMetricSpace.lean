...
@[simp]
lemma StdSimplex.map_single {R M N : Type*} [PartialOrder R] [Semiring R] [IsStrictOrderedRing R]
    (x : M) (f : M → N) : (single (R := R) x).map f = .single (f x) := by
  ext a
  simp [map, ← mk_single]

@[simp]
lemma StdSimplex.map_duple {R M N : Type*} [PartialOrder R] [Semiring R] [IsStrictOrderedRing R]
    {s t : R} (hs : 0 ≤ s) (ht : 0 ≤ t) (h : s + t = 1) (x y : M) (f : M → N) :
...
theorem convexComboPair_symm {R M : Type*} [PartialOrder R] [Semiring R] [IsStrictOrderedRing R]
    [ConvexSpace R M]
...
@[simp]
lemma dist_convexComboPair_left [IsConvexMetricSpace X]
...
lemma dist_convexComboPair_convexComboPair_le [IsConvexMetricSpace X]
...
lemma continuous_convexComboPair [IsConvexMetricSpace X] :
...
lemma continuous_convexComboPair' [IsConvexMetricSpace X] [BoundedSpace X]
